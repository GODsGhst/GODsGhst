# 👨🏻‍💻 Otávio de Pádua

**`Estudante de Engenharia de Software | Desenvolvedor Java & Python`**

Olá! Me chamo Otávio Pádua e sou estudante de Engenharia de Software, apaixonado por tecnologia, desenvolvimento de sistemas e resolução de problemas através da programação.

Atualmente desenvolvo projetos utilizando Java, Python, MySQL e PostgreSQL, buscando aprimorar continuamente meus conhecimentos em desenvolvimento backend, bancos de dados, programação orientada a objetos e arquitetura de software.

Além da área de tecnologia, também possuo experiência com empreendedorismo e gestão de processos no setor de produtos em couro legítimo, o que me proporcionou uma visão prática sobre negócios, automação e desenvolvimento de soluções para problemas reais.

Utilizo este GitHub para compartilhar projetos acadêmicos, estudos, experimentos e aplicações desenvolvidas durante minha jornada na Engenharia de Software.

<p align="left">
    <a href="https://github.com/GODsGhst?tab=followers">
        <img 
            alt="Seguidores"
            title="Me siga no GitHub"
            src="https://custom-icon-badges.demolab.com/github/followers/SEU_USUARIO?color=236ad3&labelColor=1155ba&style=for-the-badge&logo=github&label=Seguidores&logoColor=white"
        />
    </a>
    <a href="https://github.com/GODsGhst?tab=repositories&sort=stargazers">
        <img 
            alt="Estrelas"
            title="Total de estrelas"
            src="https://custom-icon-badges.demolab.com/github/stars/SEU_USUARIO?color=55960c&style=for-the-badge&labelColor=488207&logo=star&label=Estrelas"
        />
    </a>
</p>

---

## 🚀 Tecnologias

### 💻 Linguagens

<img align="left" alt="Java" width="35px" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/java/java-original.svg"/>
<img align="left" alt="Python" width="35px" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg"/>
<img align="left" alt="MySQL" width="35px" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mysql/mysql-original.svg"/>

<br/>
<br/>

### 🗄️ Banco de Dados

<img align="left" alt="MySQL" width="35px" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mysql/mysql-original.svg"/>
<img align="left" alt="PostgreSQL" width="35px" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/postgresql/postgresql-original.svg"/>

<br/>
<br/>

### 🛠️ Ferramentas

<img align="left" alt="Git" width="35px" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/git/git-original.svg"/>
<img align="left" alt="GitHub" width="35px" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/github/github-original.svg"/>
<img align="left" alt="IntelliJ" width="35px" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/intellij/intellij-original.svg"/>
<img align="left" alt="VSCode" width="35px" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/vscode/vscode-original.svg"/>

<br/>
<br/>

---

## 📚 Conhecimentos

* Programação Orientada a Objetos (POO)
* Estruturas de Dados
* Threads e Concorrência
* Modelagem de Banco de Dados
* Desenvolvimento Backend
* Engenharia de Software
* Boas Práticas de Programação
* Versionamento com Git

---

## 🎯 Objetivos

* Evoluir como Desenvolvedor Backend
* Aprofundar conhecimentos em Java e Python
* Desenvolver aplicações escaláveis
* Aprender Arquitetura de Software
* Estudar Microsserviços
* Contribuir para projetos Open Source

---

## 📈 Atualmente

* 🎓 Cursando Engenharia de Software
* ☕ Desenvolvendo projetos em Java
* 🐍 Aprimorando conhecimentos em Python
* 🗄️ Trabalhando com MySQL e PostgreSQL
* 🚀 Construindo projetos para fortalecer meu portfólio

---

## 📊 Estatísticas

<p>
  <img
    align="left"
    height="180"
    src="https://github-readme-stats.vercel.app/api?username=GODsGhst&show_icons=true&theme=tokyonight&locale=pt-br"
  />

<img
 align="left"
 height="180"
 src="https://github-readme-stats.vercel.app/api/top-langs/?username=GODsGhst&theme=tokyonight&layout=compact&custom_title=Tecnologias"
/>

</p>
